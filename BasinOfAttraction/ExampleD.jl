...
# caso con generacion solar fotovoltaica
using XLSX 
using CSV
using DataFrames 
using Plots
...
function LoadFeeder(file::String)
    ...
    IBRs = DataFrame(XLSX.readtable(file,"IBRs"))
    Radiation = DataFrame(XLSX.readtable(file,"Radiation"))
    ...
    for ibr in eachrow(IBRs)
        n_a = ibr[1]
        n_b = n_a + NumN
        n_c = n_b + NumN
        prof = ibr[4]
        Snode[n_a,:] = ibr[2]/3*Radiation[:,prof]
        Snode[n_b,:] = ibr[2]/3*Radiation[:,prof]
        Snode[n_c,:] = ibr[2]/3*Radiation[:,prof]
    end
